...
public class HorariosProyeccionDAO {
    ...
    public int agregar(HorariosProyeccion hp) {
        ...
        try {
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            ps.setInt(1, hp.getIdHorarioP());
            ps.setTime(2, hp.getHorario());
            ps.setDate(3, new java.sql.Date(hp.getFecha().getTime()));
            ps.setInt(4, hp.getDisponibilidadAsientos());
            ps.setInt(5, hp.getSala_id());
            ps.setInt(6, hp.getPelicula_id());
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
